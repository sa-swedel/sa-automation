...
#
# MON-HPSCR-NM.rb
# Test "happy path" conditions as a Monitor
# No menus
#
# Scott Wedel
# 21 January 2014
#
# Tests from the 'Monitor Script' sheet in the QC Test Modules V1.0 spreadsheet
#
####################################################################################
...
cols = ['Date', 'Subject ID', 'Visit Name', 'Form', 'Monitored', 'Actions']
temp = 0
...
salog( sep, "LOG" )
salog( 'MONITOR ROLE: \'HAPPY PATH\' AUTOMATED TESTS', "LOG" )
...
# TEST #1: Login as Monitor
tst = "1 - LOGIN"
salog( tst, "LOG" )
salog( sep, "LOG" )

tst = '1.1: Login as Monitor'
...
#while loop < menus.length
	...
	#when 0	
		...
		#0.upto( cols.length ) do |num|
			#p = b.link( :text => cols[num].to_s ).exists?
			#if ( p == FALSE )
				#salog( tst, FAIL )
				#temp +=1
				#break
		...
		# verify sorting in all columns

		#TEST 2.2 - Verify sorting in Date column	
		#tst = "2." + (loop+2).to_s + ": " + menus[loop] + ": verify sorting in Date column"

		#test_count +=1

		#b.link( :text => 'Date' ).when_present.click
		# click link again to sort (US1248)
		#sleep 3
		#b.link( :text => 'Date' ).when_present.click
...
salog( '5. CREATE QUERIES IN QUERY MANAGER', "LOG" )
salog( sep, "LOG" )
...
sleep 2
b.link( :class, "btn btn-info AddPost").when_present.click
sleep 2
...
salog( '6. VIEW & DELETE QUERIES IN QUERY MANAGER, QM COLUMN SORTING', "LOG" )
salog( sep, "LOG" )
...
test_count +=1
ex = b.text.include?( '*Please note all query fields are required.' )
if (ex == true)
	salog( tst, "PASS" )
else
	salog( tst, "FAIL" )
...
salog( '9. SET/UNSET FORM AS MONITORED', "LOG" )
salog( sep, "LOG" )

tst = '9.1: Set form as monitored'

b.link( :text => 'Set as Monitored' ).when_present.click
sleep 5
mon = b.html.include?( 'http://demo2.studyspotsa.com/img/monitor_watermark.png' )
test_count +=1
if mon == true
	salog( tst, "PASS" )
else
...
tst = '9.2: Set form as Not Monitored'

b.link( :text => 'Set as Not Monitored' ).when_present.click
sleep 2
mon = b.html.include?( 'http://demo2.studyspotsa.com/img/monitor_watermark.png' )
test_count +=1
if mon == false
